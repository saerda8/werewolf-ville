...
当前状态：已完成，并通过语法检查、观察/记忆/prompt 回归和核心状态机相关子集回归。

- [x] 观察半径采用 `10`，并按可见/可听/公共/隐藏事实权限过滤观察事件。
- [x] 决策前构建 observation packet，纳入当前周围事件、此前记忆、当前行动、目标、行动历史、公共状态、附近人物/物体和黑盒约束。
- [x] 保持三条模型线：决策线、真实聊天线、后台记忆线；后台记忆线不阻塞行动/交谈/规划，也不制造蓝泡。
- [x] 真实聊天从提交模型请求开始占用行动，到白泡消失才完成。
- [x] `thought`/`plan` 作为真实认知输出进入记忆候选，但必须限长、限视角；`action_status` 必须 grounded，不得虚构不存在对象。
- [x] observation packet 区分 10 格内可见物件和近身可操作物件；白泡 `action_status` 只用近身可操作物件兜底，避免远处物件被误写成当前行动。
...
当前状态：已完成并通过浏览器整段流程验证。

- [x] 深挖灯泡在真实交谈气泡出现时立即消失；深挖次数耗尽不影响正常交谈按钮。
- [x] 黄昏警长发言、投票和投票结果 UI 移出右侧/右下区域，统一改为屏幕中央弹窗。
- [x] 投票后按钮立即禁用，票仓显示投票者头像，并在底部单独显示弃票者头像。
- [x] 黄昏讨论禁止“先听警长/克罗”“我没意见”等划水式发言。
- [x] 投票结束后先由克罗公布最高票，再让被投者原地发表遗言，气泡停留 6 秒后再押送。
- [x] 夜晚镜头锁定克罗家，场景变暗但可见，玩家不能移动镜头。
- [x] 夜晚窗口为狼人进度条 + 银质小刀进度条两行，确认天亮可进入第二天。
- [x] 前端版本 `51`；内置浏览器已走通 Day 1 -> 黄昏 -> 投票 -> 夜晚 -> Day 2。

## 2026-06-09 NPC 真实谈话 / 晨会 / 牢房尸体 / 银器任务收口

- [x] 真实谈话期间旧行动决策线程不能再提交思考、计划、开始行动或 `_pending_action`。
- [x] Day2/Day3 晨会不再显示普通行动气泡，镜头自动回广场，NPC 静止正面，并有 15 秒到场兜底。
- [x] 被投出者押送到牢房后变成灰色躺地 `jailed_corpse`，不再参与 AI、行动或后续生活。
- [x] 五金店制造子弹工具不再依赖亚瑟存活；成功提示固定为“恭喜你获得制作子弹的工具”。
- [x] 银器资源任务文案固定为 `通过深挖女性角色获得银质项链/去五金店找到制造子弹的工具(二选一)`。
- [x] 黄昏发言 prompt 增加“被怀疑后必须回应/反踩”的约束，减少和气划水。
- [x] 验证：语法通过；相关后端回归 12 passed；晨会/投票/UI 回归 202 passed；白天真实谈话定点回归 4 passed；服务已重启，页面显示版本 `67`。
