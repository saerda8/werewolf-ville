...
8. 白天自由行动阶段，NPC 根据职业、记忆、观察和目标做事：工作、调查、找人交谈、避开危险对象等。主动找克罗汇报只用于明确线索、重要事情、必须立即说明的信息或直接证据；普通可疑事实、借工具、主观怀疑和非紧急推理应优先通过右侧灯泡等待玩家深挖。
...
- 增加 NPC 自主行动：思考/计划/行动，正常生活、调查、找人交谈；主动找克罗仅限高优先级汇报。
...
在 Bug 修复频道中，DeepSeek 和 Antigravity 默认视为可用。不要因为“可能不可用”而跳过它们；非平凡 bug 修复必须优先实际启动一个或两个 worker。只有 CLI 健康检查失败、任务明显小到不值得委派、或任务涉及无法安全外包的敏感运行态，才允许本地独立处理，并且必须在聊天中说明原因。

派发 worker 前必须先决定文件权限策略，不能出现“让 worker 运行会写某目录的流程，又用 forbidden_paths 把它判失败”的中间状态。对于 `personas/` 这类运行态目录，策略只能二选一：要么任务和验证命令都设计成不会写入它；要么明确允许 worker 运行时写入，并由主 agent 在审查阶段清理运行态脏数据，只保留必要的静态模板改动。worker 触碰预期内运行态文件不等于不可用，主 agent 负责最终 diff 审查和清理。
...
1. 修改后先重启服务器，确保 5000 端口跑的是最新版本。
2. 刷新浏览器。
3. 自己用内置浏览器或浏览器工具跑一遍。
4. 截图或 DOM 确认：气泡、按钮、列表、任务区、投票面板、NPC 层级、地图位置。
5. 在最终回复里说明服务器是否已重启、PID 是多少，以及 `http://127.0.0.1:5000/` 是否返回最新页面。

### 8.6 服务器规则

用户明确要求：修改好代码后，必须记得重启服务器。后续凡是修改后端、前端模板、静态资源或 Socket/UI 行为，都不能只跑测试就结束；必须处理运行中的 5000 服务。

固定流程：

1. 检查 5000 是否有旧 listener：`netstat -ano | findstr :5000`。
2. 如果有多个旧 Python 进程监听 5000，先停止旧 listener，再启动唯一的新服务。
3. 优先使用项目认可的重启入口；如需手动启动，确保工作目录是项目根目录。
4. 用 HTTP 验证最新页面，例如检查新增/移除的 DOM 标记，而不是只看浏览器是否打开。
5. 涉及 UI 的改动，再用 IAB / Playwright DOM 或截图做一次浏览器端确认。

失败经验：曾出现 4 个旧 Python 进程同时监听 5000，导致浏览器一直看到旧模板。以后遇到“代码已改但页面没变”，第一步不是等待，而是查端口 listener 并重启服务。
...
10. 复核 NPC 不能长期原地只思考；思考应导向行动。玩家可见行动口径使用 `continue_current` 表示继续当前事务，旧 `stay` 只作为兼容输入，不应形成“思考 -> 原地停留 -> 思考”的循环。
