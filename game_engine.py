...
def get_nearby_objects(x, y, radius, sector_maze, arena_maze, go_maze, sector_dict, arena_dict, go_dict):
    ...
    for dy in range(-radius, radius + 1):
        for dx in range(-radius, radius + 1):
            nx, ny = x + dx, y + dy
            info = get_tile_scene(nx, ny, sector_maze, arena_maze, go_maze,
...
GATHERING_FALLBACKS = {
    ...
    "Arthur Burton": "昨晚我在店里修工具。听到命案后，我心里很不安。",
    "Isabella Rodriguez": "昨晚我在咖啡馆收拾到很晚。今早的事让我害怕。",
    "Klaus Mueller": "昨晚我在学院看资料。发生命案后，我会配合调查。",
    "Maria Lopez": "昨晚我在整理货物。现在有人死了，我不敢隐瞒。",
    "Sam Moore": "昨晚我在家休息。若有人听见动静，我也想知道。",
    "Jane Moreno": "昨晚我在公园边整理花坛到很晚。发生这样的事，我很担心。",
    "Mei Lin": "昨晚我在图书馆整理文献到很晚。命案让我很震惊。",
}
        ...
        if self._gathering_busy:
            ...
            if speak_elapsed > self.gathering_turn_timeout:
                ...
                with self._lock:
                    ...
                    gap = (
                        self._gathering_departure_gap_seconds()
                        if self._gathering_round >= 2
                        else self._gathering_speech_visible_seconds()
                    )
...
当前场景：现在是早晨，所有居民聚集在尸体附近。大家正在轮流发言，每人说一两句话。镇上刚发现命案，气氛应当严肃、紧张、克制。
...
## 任务（第一轮发言 — 只能说话，不能离开）
请发表你的第一轮发言。不要自我介绍，不要说”我是……”。直接说 1-2 句完整的话（60-72字左右，完整句可到约75字），谈谈：
- 昨晚你在哪里，做了什么
- 你现在心里最不踏实的是什么
- 你今天打算做什么
- 本次发言角度：{focus_text}
...
9. 直接输出你的发言文本，不要返回 JSON 格式。"""
        ...
        def _do_speak():
            try:
                ...
                self._gathering_speaker_idx += 1
                self._gathering_next_tick = time.time() + self._gathering_speech_visible_seconds()
                self._gathering_busy = False
...
不要编造小镇不存在的人、地点或物件。不要在这一步公开指控别人，只解释自己接下来要去做的事。"""
    ...
    def _transition_to_dusk(self):
        ...
        self.phase = GamePhase.DUSK_DISCUSSION
...
{{"reason": "你的推理（简短）", "vote": "指控对象名（可选对象之一，或空置票）"}}"""
            ...
            raw = ""
            try:
                raw = chat_for_agent(voter_name, system_prompt, "请投票指控一个嫌疑人", max_retries=0)
    ...
    def _jailed_final_words(self, target_name: str) -> str:
        ...
        try:
            raw = chat_for_agent(target_name,
    ...
    def _build_daily_tasks(self) -> list[dict]:
        ...
        Day 1: Interview all living non-jailed non-Crow residents.
        Every day: Interview requirement.
        Day 2+: Silver supply progression — one key objective per day:
          - Obtain silver-bullet tool from Arthur at Harvey Oak Supply Store.
          - Obtain silver jewelry from the hidden jewelry holder.
        Day 3+: Which is still missing.
        Day 4+: If both acquired, expose optional craft/use silver bullet task.
        """
        tasks = []
        # 1. Interview task (every day)
        required_interviews = {n for n, a in self.agents.items()
                               if n != self.detective_name and a.is_alive and n not in self._jailed}
        ...
        # 2. Silver tasks (Day 2+)
        if self.day >= 2:
            # Silver bullet from Arthur
            ...
            # Day 4+: Craft silver bullet if both acquired
                ...
                try:
                    ...
                    def _fallback_stay(reason: str):
                        ...
                        self._log(
                            ...
                            f"行动={a.current_action}；类型=stay；原因={reason}",
                            "think",
                        )

                    if not decision.get("ok", False):
                        err = decision.get("error", "unknown")
                        self._log(f"[LLM状态] {actor_label}: 决策失败（{err}），转为stay，稍后重试", "think")
                        _fallback_stay(f"模型暂时没有给出可执行计划：{err}")
                    ...
                    if not target_person and self._decision_mentions_detective(
                        loc, target_obj, action, thought, expected_result
                    ):
                    ...
                    # 不需移动（原地执行）的动作类型
                    _IN_PLACE_ACTIONS = {"stay", "observe", "work", "rest", "hide", "socialize", "talk", "inspect", "investigate"}
                    ...
                    if not loc:
                        if action_type in _IN_PLACE_ACTIONS:
                            ...
                            self._log(f"[LLM状态] {actor_label}: 模型未返回有效地点（类型={ACTION_TYPE_LABELS_ZH.get(action_type, action_type)}），转为stay，稍后重试", "think")
                            _fallback_stay("模型没有说明要去哪里")
                            a._next_llm_retry_time = time.time() + CONFIG.get("llm", {}).get("retry_delay_seconds", 5)
                            return
                    ...
                    if (
                        str(action_type).lower() in {"stay", "observe"}
                    ...
                    if action_type in {"move_to", "inspect", "investigate", "work"} and not target_person and not target_obj:
                        ...
                        if not target_obj:
                            self._log(
                                f"[LLM状态] {actor_label}: 行动缺少可验证的真实物件（地点={self._destination_label_zh(matched_loc)}），转为stay，稍后重试",
                                "think",
                            )
                            _fallback_stay("模型没有给出具体可交互物件且无默认物件可用")
                            a._next_llm_retry_time = time.time() + CONFIG.get("llm", {}).get("retry_delay_seconds", 5)
                            return
                        self._log(
                    ...
                    if target_person:
                        ...
                        if not available:
                            ...
                            _fallback_stay(f"目标暂时不适合交谈：{busy_reason}")
                            a._next_llm_retry_time = time.time() + CONFIG.get("llm", {}).get("retry_delay_seconds", 5)
                            return
                    ...
                    # 保存 pending action
                    a._pending_action = {
                        "action_type": action_type or "stay",
                    ...
                    a.current_action_type = action_type or "stay"
                    a.current_emoji = self._get_emoji(a.current_action)
    ...
    def _path_adjacent_to(self, start, target, blocked=None):
        ...
        # 1. Collect walkable tiles adjacent to the target (4-directional)
        adjacent = []
        for dx, dy in [(1, 0), (-1, 0), (0, 1), (0, -1)]:
            nx, ny = tx + dx, ty + dy
        ...
        # 3. Choose the adjacent tile with the shortest reachable path
        best = None
    ...
    def _complete_agent_action(self, name: str, agent) -> None:
        ...
        action_type = pending.get("action_type", "stay")
        ...
        if action_type in ("talk", "socialize") and target_person:
            target_agent = self.agents.get(target_person)
            if target_agent and target_agent.is_alive and self._agent_distance(agent, target_agent) <= 1:
                if target_person == self.detective_name:
                    # Guard: check Crow not already in conversation
                    detective = self.agents.get(self.detective_name)
                    if detective and getattr(detective, "in_conversation_with", None) is not None:
                        ...
                        agent._pending_action = None
                        return
                    # Clear stale thought/plan for NPC before talking to Crow
    ...
    def _agent_has_visible_clue_hint(self, name: str, agent) -> bool:
        ...
        if not isinstance(last_decision, dict):
            last_decision = {}
        target_person = pending.get("target_person") or last_decision.get("target_person") or ""
        if resolve_character_name(str(target_person)) == self.detective_name:
            return False
        return self._text_suggests_new_clue(
            getattr(agent, "current_thought", ""),
            getattr(agent, "current_action", ""),
            pending.get("thought", ""),
            pending.get("expected_result", ""),
            last_decision.get("thought", ""),
            last_decision.get("expected_result", ""),
        )
    ...
    def _clear_conversation_if_too_far(self, name1: str, name2: str) -> bool:
        ...
        if not a1 or not a2:
            return False
        if self._agent_distance(a1, a2) <= 1:
            return False
    ...
    def _trigger_npc_chat(self, name1: str, name2: str):
        ...
        with self._lock:
            ...
            if not agent1.is_alive or not agent2.is_alive:
                return
            if self._agent_distance(agent1, agent2) > 1:
                return
...
规则：1. 保持角色性格 2. 自然对话 3. 简短（1-2句话，60-72字左右，完整句可到约75字）"""
                ...
                with self._lock:
                    if (
                        ...
                        or self._agent_distance(agent1, agent2) > 1
                    ):
                        return
                if not msg1:
                ...
                with self._lock:
                    if self._npc_chat_tokens.get(chat_key) != token or self._agent_distance(agent1, agent2) > 1:
                        return
                    self.chat_bubbles.pop(name1, None)
                    self.chat_bubbles.pop(name2, None)
    ...
    def get_status(self) -> dict:
        with self._lock:
            ...
            for name, agent in self.agents.items():
                ...
                "action": "" if is_detective else agent.current_action,
